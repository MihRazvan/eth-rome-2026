...
export function purchaseQuoteCodec(expected: { claimId: bigint; source: Address; sourceVersion: bigint }, client?: PublicClient): QuoteCodec<SignedQuote> {
  return {
    ...
    verify: async (value, context) => context.requestId === offerRequestId(context.chainId, context.market, value.quote.claimId, value.quote.ownershipEpoch) &&
      value.quote.seller.toLowerCase() === context.seller.toLowerCase() &&
