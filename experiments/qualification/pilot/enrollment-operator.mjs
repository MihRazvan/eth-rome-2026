...
import {generateChannelKeys,openEnrollment,sealEnrollment,parseApplication,parseApproval,verifyEnrollmentAuthorization,parseSealedEnrollment} from './enrollment-channel.ts';
...
if(command==='init'){
 ...
 if(command==='list'){
  ...
  if(record.digest!==digest || !await verifyEnrollmentAuthorization(record.applicant,context,digest,record.expires,record.signature))throw Error('Application wallet authorization failed');
