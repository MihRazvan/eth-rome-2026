...
export function mountCutoutDemo(container: HTMLElement): () => void {
  ...
  function render(focus = true) {
    ...
    if (step === 2)
      panel.innerHTML = `<p class="cd-eyebrow">03 / SWITCH HATS</p><h3 tabindex="-1">Accept the task.<br><em>Keep the rest.</em></h3><p class="cd-lead">You are now the reviewer. The live app sets up private reviewer access automatically, then proves it for this task. No team approval or file exchange.</p><div class="cd-credential"><div class="cd-person">${person}</div><div><span class="cd-kicker">PRIVATE ACCESS</span><h4>Open participation</h4><p>Created automatically in the live app</p><label class="cd-toggle"><input type="checkbox" data-demo-valid checked> Credential is currently valid</label></div><span class="cd-stamp">LOCAL<br>PREVIEW</span></div><div class="cd-visibility"><div><span class="cd-kicker">THE CLIENT LEARNS</span><p>Your participation pass is current.</p></div><div><span class="cd-kicker">KEPT OUT OF THE PROOF</span><p>Your reusable credential identifier.</p></div></div><p class="cd-note">This is an eligibility simulation. The live workspace generates a zero-knowledge proof; this demo does not. Public wallet activity can still link a reviewer’s jobs.</p>${button("qualify", "Accept this task ↗")}`;
    if (step === 3)
