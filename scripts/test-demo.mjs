...
const server = await createServer({
  ...
  server: { host: "127.0.0.1", port: 18920, strictPort: true },
});
await server.listen();
...
try {
  for (const width of [1440, 390, 320]) {
    ...
    await page.screenshot({
      path: `${dir}/receipt-${width}.png`,
      fullPage: true,
    });
    await page.locator('[data-demo="restart"]').click();
    await page.locator("#cd-title").waitFor();
    checks.push(
      `${width}px: entry, rejected invalid demo eligibility, exact encryption/decryption, simulated payment, restart; no overflow`,
    );
    await page.close();
  }
    ...
    JSON.stringify(
      {
        ...
        errors,
        scope:
          "Local browser walkthrough. No chain/storage writes or live proof.",
      },
      null,
      2,
    ) + "\n",
  ...
  console.log(
    `Deaddrop browser demo: ${checks.length} viewport checks passed.`,
  );
} finally {
  await browser.close();
  await server.close();
