...
A final independent judge-entry evaluation found absolute filesystem paths in missing-asset errors. Revision `dea5ba2` returns generic404 for absent/inaccessible files. Five actual HTTP probes verified that private/unsupported paths are not served and no filesystem path is returned. The empty public Arkiv board now links explicitly to the separate local rehearsal task. Independent screenshots and root read-only checks confirmed the real open scope, persisted role selection, missing-wallet help and390px/focus behavior without consuming the task.
