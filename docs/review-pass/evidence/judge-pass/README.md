...
Reproduce commands are in the [runbook](https://github.com/MihRazvan/eth-rome-2026/blob/c50937c/docs/review-pass/RUNBOOK.md). The browser test intentionally revokes its credential and leaves its snapshot stale. The subsequently prepared manual runtime uses a fresh credential and separate escrow; it does not reset the chain, erase allocation/revocation history or change completed evidence. Manual task 1 is a real local 250 qUSD escrow with a 24-hour acceptance window from its funding block. The user still needs a local test wallet and browser device key; enrollment instructions are in [provisioning](https://github.com/MihRazvan/eth-rome-2026/blob/c50937c/docs/review-pass/QUALIFICATION-PROVISIONING.md).
