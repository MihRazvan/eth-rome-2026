...
- [ ] Confirm the reviewer's current **class-7 credential JSON** and matching **private holder JSON** are available locally. This is the remaining assisted enrollment prerequisite: a wallet alone cannot obtain these through the app. Engineering handles issuer provisioning; the holder's secret stays on the holder's device. [Enrollment details](review-pass/QUALIFICATION-PROVISIONING.md).
