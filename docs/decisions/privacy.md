...
HPKE info fixes application/protocol/suite; AEAD associated data binds request ID, seller, chain, market, key hash/version, envelope format/version. Sign the canonical purchase quote first, verify it, then encrypt quote plus signature. The request codec validates seller, claim, source revision, quote EIP-712 domain and deterministic request ID derived from chain/market/claim/ownership epoch. A correctly signed old quote cannot be repackaged under a new ownership request. Current onchain validity, exact depletion bounds, funds and cancellation are settlement checks, so a decrypted signature is not displayed as guaranteed executable.
