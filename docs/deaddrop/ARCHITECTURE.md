# Deaddrop — technical architecture
...
The credential signature, actual credential expiry, holder secret/commitment, revocation index and Merkle path are private witnesses. Full details are in the [circuit](../../experiments/qualification/prover/circuit.go) and [qualification workflow](QUALIFICATION.md). The wallet and task remain public, so reusing a wallet still links activity.
...
An Arkiv listing advertises an existing funded task; it does not create the financial claim. Compound queries select matching public listings. Deaddrop authenticates the entity owner and validates task data/eligibility against finalized Fuji state before offering acceptance.
