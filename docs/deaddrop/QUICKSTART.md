...
Run on a clean local development environment. These scripts use public Anvil development accounts and valueless local tokens. Keep the servers loopback-bound. Existing shared runtimes must not be reset during another participant’s manual test.
...
Leave it running. It builds a local experimental proving setup and matching verifier, starts Anvil **31338 on port 18547**, uploads a local issuer snapshot and runs its integration scenario. It writes ignored artifacts under `.runtime/qualification/`. Port 18787 serves the earlier feasibility workbench; the current Deaddrop UI is started in the next step. Startup requires actual local Bee; storage failures stay failures.

**3. Deploy and serve Deaddrop on that local runtime.** In a second terminal, from the repository root, use the same batch ID:

```sh
export QUALIFICATION_LOCAL_POSTAGE='REPLACE_WITH_LOCAL_BATCH_ID'
export QUALIFICATION_PILOT_DIR=.runtime/deaddrop-local
...
```sh
QUALIFICATION_PILOT_DIR=.runtime/deaddrop-local QUALIFICATION_PILOT_PORT=18889 node experiments/qualification/pilot/browser-test.mjs
```
...
For a fresh local pilot after that test, redeploy the pilot and restart its server; preserve issuer allocation history. Stop only your own processes using their terminal's Ctrl+C. Stopping the underlying runtime stops its Anvil. Docker containers are separate; do not remove unrelated containers.
...
Use [Deaddrop](https://cutout-ethrome-2026.vercel.app) and the [manual test tutorial](TESTING.md). Client: Fuji test AVAX, canonical test USDC and Tiramisu test GLM. Reviewer: Fuji test AVAX, a current saved class-7 reviewer pass. Both need their own registered browser report keys. Swarm storage is included without an account.
