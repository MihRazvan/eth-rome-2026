...
A generated 1,625-byte recipient-encrypted report was actually uploaded to public Swarm and separately retrieved. Both test recipients recovered exact Unicode text; reload preserved the key; an unrelated browser could retrieve ciphertext but not decrypt. Production-origin CORS/CSP upload checks also passed. These used generated test document bindings, not a completed funded report-delivery lifecycle.

## Show the judges

Seal a recognizable non-sensitive report, show its content reference and successful retrieval check, and open it from the client's original browser. Compare the plaintext exactly and demonstrate an unrelated browser cannot decrypt. Then show the Fuji delivery commitment for the paid task once that public lifecycle has been completed.

Separate retrieval verifies availability and integrity at that time; the current upload and retrieval paths use the same gateway operator. Temporary gateway storage has unspecified retention and service limits. Cutout offers local report export. Our production next step is an operator-funded gateway with a stated retention policy, followed by a real review pilot. We do not claim permanent storage or independent gateway operators.
