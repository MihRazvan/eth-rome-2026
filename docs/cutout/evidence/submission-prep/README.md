...
The root README and docs index now describe Cutout, with a separate product brief, quickstart, user flow, architecture, trust model, deployment, sponsor pages and evidence map. The earlier EXIT README is preserved in `docs/history/EXIT.md`. Original application paths and protocol identifiers are unchanged.
