...
Follow-up: `App.run` discards late result/error notices when wallet/network identity no longer matches the action's starting identity. This prevents an old simulation error from repopulating cleared UI with earlier terms after an account switch. The original in-flight operation still clears its generic busy indicator when it finishes; the controller is responsible for preventing stale authorization/execution and reconciling actual chain outcomes. Five browser checks and TypeScript were rerun after this guard.
