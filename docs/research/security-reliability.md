...
Integrator commit `42bd712` with shared foundation `7e94920` was then cherry-picked into the isolated review worktree. `node --import tsx docs/research/repros/corrected-boundaries.ts` passed independent reversed-expectation probes. Real HPKE/signature inputs remained in use: old seller refreshes no longer committed private prices or ownership into the maker session; API context substitution was rejected before key loading; discovery failure retained the authoritative position; an existing valid certificate was republished.

Two different private bids produced the same fixed-capital approval arguments/calldata, and a third with sufficient allowance produced no approval. Decoding the approval calldata confirmed that its amount was not either confidential bid. Missing recipient keys and over-budget offers failed before approval/signing. The result file is `repros/corrected-boundary-results.json`. These are controller/cryptography probes with stubbed chain responses, not public transactions; the integrator's browser/onchain approval evidence is a separate gate. Earlier private-mode acceptance evidence predating this fix must not be read as proving that the approval side channel was absent.

The runtime correction also passed eight isolated HTTP/queue tests, including a real unfinished client socket disconnect with the server's `aborted` event observed and a subsequent successful request. No hypothesized abort-listener crash reproduced on Node v25.2.1, so that follow-up changed tests only. A combined targeted run of controller, boundary and runtime suites passed 23 tests. Pending-transaction recovery across reload remains distinct from the corrected replacement and session boundaries.
